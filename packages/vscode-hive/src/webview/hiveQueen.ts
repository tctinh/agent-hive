...
type PanelMode = 'planning' | 'execution';

const vscode = acquireVsCodeApi();

let currentMode: PanelMode = 'planning';
...
function renderPlanningView(content: string, title: string): void {
...
function renderExecutionView(tasks: TaskProgress[]): void {
  const planContent = document.getElementById('plan-content');
  if (!planContent) return;
...
function renderSubtasks(subtasks: { id: string; name: string; status: string }[]): string {
...
function renderComments(): void {
  ...
  if (comments.length === 0) {
    const strings = (window as any).__HIVE_QUEEN_STRINGS__ || {};
...
function openCommentDialog(lineNumber: number): void {
...
function openEditDialog(index: number): void {
  ...
  if (commentInput) { commentInput.value = comments[index].text; commentInput.focus(); }
...
function showAskDialog(ask: PendingAsk): void {
  currentAskId = ask.id;
  
  const dialog = document.getElementById('ask-dialog');
  const questionEl = document.getElementById('ask-question');
  const answerInput = document.getElementById('ask-answer') as HTMLTextAreaElement;
