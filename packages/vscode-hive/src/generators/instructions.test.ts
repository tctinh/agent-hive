...
describe('instructions generator', () => {
  ...
  it('builds the workflow instructions with frontmatter and short always-on body', () => {
    ...
    expect(body).toContain('Load only the skill or tool guidance the current task triggers');
    expect(body).toContain('browser tools for web or UI inspection');
    expect(body).toContain('Playwright MCP for browser automation or end-to-end verification');
    expect(body).toContain('Copilot memory for durable notes needed across turns');
